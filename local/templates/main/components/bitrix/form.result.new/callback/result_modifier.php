...
foreach ($arResult["QUESTIONS"] as $FIELD_SID => &$arQuestion)
{
	...
	if ($FIELD_SID  =="service" && $arParams["SERVICE"])
	{
		$arQuestion["HTML_CODE"]=str_replace('value="', 'value="'.htmlspecialchars($arParams["SERVICE"]), $arQuestion["HTML_CODE"]);
	}
	if ($FIELD_SID  =="product" && $arParams["PRODUCT"])
	{
		$arQuestion["HTML_CODE"]=str_replace('value="', 'value="'.htmlspecialchars($arParams["PRODUCT"]), $arQuestion["HTML_CODE"]);
	}
}
?>