    ...
    constructor(target) {
        ...
        this.tabsNav?.querySelectorAll('[data-tabs-id]').forEach(btn => {
            btn.addEventListener('click', e => {
                ...
                const navElement = document.querySelector('.works-slider__nav');

                if (currentBtn.closest('.selected')) return;

                this.tabsNav.querySelector('.selected').classList.remove('selected');
                currentBtn.classList.add('selected');

                if (currentBtn.dataset.tabsId === 'tab-list') {
                    navElement?.style.setProperty('display', 'block');
                } else {
                    navElement?.style.setProperty('display', 'none');
                }

                this.changeActiveTab(currentBtn.dataset.tabsId);
            });
        });
    }
    setup() {
